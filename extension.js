...
function activate(context) {
	context.subscriptions.push(vscode.workspace.onWillSaveTextDocument(() => {
		...
		imports.forEach(line => {
			...
			if (line.includes(`${orgName}/${repoName}`)) {
				for (const category in categories.self) {
					for (const alias of (category_alias_map[category] || [category]))
						if (line.includes(alias)) return categories.self[category].push(line)
				}
			...
			if (line.includes(orgName)) {
				for (const category in categories.sister) {
					for (const alias of (category_alias_map[category] || [category]))
						if (line.includes(alias)) return categories.sister[category].push(line)
				}
		...
		const processCategory = (name, category) => {
			...
			if (category instanceof Object) Object.entries(category).forEach(([key, value]) => result += processCategory(key, value))
